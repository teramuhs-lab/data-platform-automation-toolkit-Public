...
EXPECTED_SCHEMA = {
    ...
    "tables": {
        "dbops.migration_history": [
            "id", "version", "script_name", "checksum",
            "applied_on", "applied_by", "execution_ms", "success",
        ],
        "inventory.environments": [
            "environment_id", "name", "description", "is_production", "created_at",
        ],
        "inventory.servers": [
            "server_id", "hostname", "instance_name", "port", "environment_id",
            "edition", "version", "is_ag_primary", "last_checked", "notes",
            "created_at", "updated_at",
        ],
        "inventory.databases": [
            "database_id", "server_id", "name", "recovery_model",
            "compatibility", "size_mb", "owner", "is_monitored", "created_at",
        ],
        "inventory.backup_history": [
            "backup_id", "database_id", "backup_type", "backup_path",
            "size_mb", "compressed", "verified", "started_at", "completed_at",
            "duration_sec", "initiated_by",
        ],
        "inventory.alert_rules": [
            "rule_id", "name", "metric", "operator", "threshold",
            "severity", "is_enabled", "created_at",
        ],
        "inventory.incidents": [
            "incident_id", "rule_id", "server_id", "database_id",
            "severity", "message", "detected_at",
...
def run_drift_check(config_path: str, target_database: str | None = None) -> None:
    ...
    for table_name, expected_cols in EXPECTED_SCHEMA["tables"].items():
        if table_name not in live_tables:
            drifts.append({"type": "MISSING_TABLE", "object": table_name,
    ...
    if drifts:
        ...
        if is_json_mode():
            add_json_result("drift_check", "drift", {"drifts": drifts, "count": len(drifts)})
            flush_json()
